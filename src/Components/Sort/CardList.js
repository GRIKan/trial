...
const CardList = ( {changelang, inDrag, setInDrag, setSure, setCategoryInfo, setCreate, 
    setDrop, setOut, setNewId1, setNewId2, setOldCategory, setOldIndex, setDataCard, setDataPosition, setPop, 
    cards, cardsInCategory, categories, categoriesOrder, setCards, setCardsInCategory, setCategories} ) => {
        ...
        const handleOnDragEnd = (result) => {
            ...
            if (destination.droppableId.includes('card')) {
                setCreate(true);
                setDrop(true);
                setNewId1(draggableId);
                setNewId2(destination.droppableId);
                setDataPosition(document.querySelector('#scrollbar').scrollTop);
                if (source.droppableId.includes('category')) {
                    setOut(true);
                    setOldCategory(source.droppableId);
                    setOldIndex(source.index);
                    return;
                }
                else {
                    setOut(false);
                            ...
                            <Masonry
                                ...
                                columnClassName= "my-masonry-grid_column">
                                    {categoriesOrder.order.map((categoryId) => {
                                        ...
                                        return (
                                            <Category key= {category.id} displaydrops= {displaydrops}
                                    ...
                                    {Object.values(cards).map((card, index) => {
                                        return (
                                            <Card key= {card.id} index= {index} displaydrops= {displaydrops}
                                                changelang= {changelang}
                                                setCreate= {setCreate} setDrop= {setDrop}
                                                setDataCard= {setDataCard} setDataPosition= {setDataPosition} 
                                                setPop= {setPop} 
